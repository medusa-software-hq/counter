...
/**
 * All persisted per-environment state under a single [Environment.configDir]. Today that is just the
 * cached sign-in ([Credentials]); this is the one place that reads or writes the config directory,
 * so its file layout and on-disk permissions live in exactly one spot.
 *
...
class ConfigStore(private val dir: Path) {
  ...
  fun saveCredentials(credentials: Credentials) {
    if (!Files.exists(dir)) {
      runCatching { Files.createDirectory(dir, DIR_PERMISSIONS) }.getOrElse {
        Files.createDirectories(dir)
      }
    }
    Files.deleteIfExists(credentialsFile)
    runCatching { Files.createFile(credentialsFile, FILE_PERMISSIONS) }.getOrElse {
